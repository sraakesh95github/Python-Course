...
# Open a file object to write the test data
file = open(r"Test Results.txt", 'r+')  # The r represents a raw string.
...
for hidden_layers in HIDDEN_LAYER_SIZES:
    ...
    hl_plot_title = ''  # hidden layer plot title
    for i in range(len(hidden_layers)):
        hl_plot_title = hl_plot_title + str(hidden_layers[i]) + ', '
    plt.title("Accuracies for PCA components with Hidden layer size: (" + hl_plot_title + ')')
    # plt.show()
    ...
    plt.savefig("plot" + str(loop_counter) + ".png")

    # Write the result into a test file
    write_str = "\nHidden Layer size: " + hl_plot_title + "\nThe #components for \
        maximum accuracy: " + str(max_index) +\
        "\nMax accuracy: " + str(max_acc_test)
    # r+ reads the file first and positions the cursor at the beginning of the file
